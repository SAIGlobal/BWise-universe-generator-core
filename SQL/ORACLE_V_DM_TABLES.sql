...
(
  ...
  SELECT
    ...
    ,C.NAME as CLASSNAME
    ,C.SUPERCLASS
    ,connect_by_root(C.CLASSDEFID)as CHILDID
    ,connect_by_root(C.NAME) as CHILDNAME
  ,c.CUSTOMIZED

  FROM M_CLASSDEF C
  
  connect by     CLASSDEFID  =  prior SUPERCLASS
...
ATTRIBUTES AS (
  ...
  ,rec.CUSTOMIZED 
     ...
     ,  CASE WHEN A.UPPERMULTIPLICITY > 1 AND T.DISCRIMINATOR = 'INSTANCE' THEN UPPER(REC.CHILDNAME) || '_' || UPPER(O.NAME) ELSE  UPPER(REC.CHILDNAME) END TABLENAMERAW
  FROM REC
  ...
  LEFT JOIN (
    SELECT 'INSTANCE' TYPE, 'FOREIGN_KEY' NEWTYPE, 'ID' COLUMNTYPE, 'ID' SUFFIX, 1 SEQUENCENUMBER FROM DUAL
    UNION ALL SELECT 'INSTANCE' TYPE, 'STRING' NEWTYPE, 'VALUE' COLUMNTYPE, 'NAME' SUFFIX, 0 SEQUENCENUMBER FROM DUAL
    UNION ALL SELECT 'INSTANCE' TYPE, 'STRING' NEWTYPE, 'TYPE' COLUMNTYPE, 'TYPE' SUFFIX, 2 SEQUENCENUMBER FROM DUAL
  ) TMP
...
TABLES AS (
  SELECT DISTINCT
    ...
    ,ATTR.ROOTCLASS
    ,ATTR.TABLENAME
    ,CASE WHEN ATTR.ISMULTIPLE = 1 THEN 'CrossTable' ELSE 'DimensionTable' END TABLETYPE
    ,UPPER(ATTR.CLASSNAME) || 'ID' PKCOLUMN
    ,CASE WHEN ATTR.ISMULTIPLE = 1 THEN -90 ELSE -100 END SEQUENCENUMBER
  , ATTR.CUSTOMIZED
    , ATTR.TABLENAMERAW
  ...
  SELECT DISTINCT
    ATTR.CLASSNAME
    ,ATTR.ROOTCLASS
    ,'N_' || UPPER(ATTR.CLASSNAME) TABLENAME
    ,'HierarchyTable' TABLETYPE
    ,'CHILDID' PKCOLUMN
    ,-80 SEQUENCENUMBER
  , ATTR.CUSTOMIZED
        , ATTR.TABLENAMERAW
  ...
  WHERE ATTR.ROOTCLASS IN (
    'AbstractFolder',
    'AbstractCategory',
    --'AbstractGroup',
    'ProcessHierarchy'
  )
  OR ATTR.CLASSNAME IN (
    'Group'
  )
),
...
ALL_TABLES AS (
  SELECT
    ...
    ,CASE WHEN T.TABLENAME LIKE 'T_BW%' OR T.TABLENAME LIKE 'X_BW%' THEN 0 ELSE 1 END ISHISTORIC
  FROM TABLES T

)

SELECT "CLASSNAME","ROOTCLASS","TABLENAME","TABLETYPE","PKCOLUMN","SEQUENCENUMBER","CUSTOMIZED","TABLENAMERAW","DWHTABLENAMESUFFIX","ISHISTORIC" FROM ALL_TABLES
--where classname = 'ControlMeasure'
--order by 1;